...
def runGridSearch(params_grid=None):
    ...
    grid = GridSearchCV(sgd, param_grid=params_grid, cv=5, verbose=5, n_jobs=-1)
    grid.fit(X, Y[:, 1])
    return grid.best_estimator_


def runSGDWithParams(sgd, scorer=None):
    if scorer is None:
        scorer = make_scorer(accuracy_score)
    X = np.loadtxt("./OutputDir/window_matrix.csv", delimiter=",", dtype=float)
    Y = np.loadtxt("./OutputDir/window_matrix_terms_labeled.txt", delimiter=",", dtype=str)
    scores = cross_val_score(sgd, X, Y[:, 1], cv=5, scoring=scorer)
    return np.mean(scores)

def plotConfusionMatrix(sgd):
    X = np.loadtxt("./OutputDir/window_matrix.csv", delimiter=",", dtype=float)
    Y = np.loadtxt("./OutputDir/window_matrix_terms_labeled.txt", delimiter=",", dtype=str)
    from sklearn.model_selection import train_test_split
    data_train, data_test, labels_train, labels_test, couples_train, couples_test = train_test_split(X, Y[:, 1],
                                                                                                     Y[:, 0],
                                                                                                     test_size=0.20)
    ...
    plt.show()
    return confusion_matrix(labels_test, y)
